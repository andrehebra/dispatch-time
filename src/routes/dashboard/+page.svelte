<script>
  ...
  let tempNextDate = new Date(
    periodStartDate.getTime() + 13 * 24 * 60 * 60 * 1000
  );
    ...
    if (earliestDate <= nextPeriod) {
      periodArray.push({
        ...
        name:
          ...
          " - " +
          tempNextDate.toLocaleDateString("en-US"),
      });
    }
  ...
  function calculatePayHours() {
    ...
    let endOfPeriod = new Date(
      selectedPeriod.getTime() + 14 * 24 * 60 * 60 * 1000
    );
    ...
    for (let i = 0; i < adminUsers.length; i++) {
      if (adminUsers[i].Name != null) {
        ...
        for (let j = 0; j < adminUsers[i].times.length; j++) {
          if (
            adminUsers[i].times[j].clockIn.toDate() >= selectedPeriod &&
            adminUsers[i].times[j].clockIn.toDate() <= endOfPeriod
          ) {
            if (adminUsers[i].times[j].clockOut != "CURRENTLY CLOCKED IN") {
              totalHours +=
                Math.abs(
                  adminUsers[i].times[j].clockOut.toDate() -
                    adminUsers[i].times[j].clockIn.toDate()
                ) /
                (1000 * 60 * 60);
            }
          }
  ...
  authStore.subscribe((curr) => {
    ...
    if (curr.user != null) {
      if (curr.user.uid != null) {
        if (
          curr.user.uid == "691DpeS3ATWdkFygcrC0cefMxLl1" ||
          curr.user.uid == "PzHTuv5VaAdMQsgp0RFGSlJR9lJ2"
        ) {
          admin = true;
          getAllUserTimes();
        }
  ...
  async function clockIn() {
    ...
    if (currentLocation == null) {
      alert(
        "Unable to clock in, you must allow location access for this tool to work."
      );
      return;
    }

    if (
      timeMapArray.length != 0 &&
      timeMapArray[timeMapArray.length - 1].clockOut == "CURRENTLY CLOCKED IN"
    ) {
  ...
  function clockOut() {
    ...
    if (
      ...
      timeMapArray[timeMapArray.length - 1].totalTime =
        Math.abs(
          timeMapArray[timeMapArray.length - 1].clockOut -
            timeMapArray[timeMapArray.length - 1].clockIn.toDate()
        ) /
        (1000 * 60 * 60);
      saveTodos();
    } else {
  ...
  async function scrollToBottom() {
    const personContainers = document.getElementsByClassName("personContainer");
    ...
    <main>
      {#if admin === false}
        ...
        {#each timeMapArray as clockInItem, index}
          <div class="todo">
            <p>
              {index + 1}. {clockInItem.clockIn
                .toDate()
                .toLocaleDateString("en-US")}
              {clockInItem.clockIn.toDate().toLocaleTimeString("en-US")} - {clockInItem.clockOut ==
              "CURRENTLY CLOCKED IN"
                ? "CURRENTLY CLOCKED IN"
                : clockInItem.clockOut.toDate().toLocaleDateString("en-us")}
              {clockInItem.clockOut == "CURRENTLY CLOCKED IN"
                ? ""
                : clockInItem.clockOut.toDate().toLocaleTimeString("en-us")}
            </p>
          </div>
        {/each}
      {:else if admin === true}
            ...
            <div class="personContainer">
              {#each user.times as clockInItem, index}
                {#if clockInItem.clockIn
                  .toDate()
                  .getHours() != 5 && clockInItem.clockIn
                    .toDate()
                    .getHours() != 13}
                  <div class="todo late">
                    <div class="horiz-list">
                      ...
                      {#if clockInItem.clockOut != "CURRENTLY CLOCKED IN"}
                        <input
                          bind:value={clockInItem.clockOutDate}
                          type="date"
                        />
                        <input
                          bind:value={clockInItem.clockOutTime}
                          type="time"
                        />
                      {:else}
                      ...
                      {#if clockInItem.location != null && clockInItem.location[0] != null && clockInItem.location[1] != null}
                        <a
                          href={"https://maps.google.com/?q=" +
                            clockInItem.location[0] +
                            "," +
                            clockInItem.location[1]}>View Location</a
                        >
                      {/if}
                      ...
                      <a
                        on:click={() => {
                          updateClockIn(
                            ...
                            index,
                            new Date(
                              clockInItem.clockInDate +
                                " " +
                                clockInItem.clockInTime
                            ),
                            new Date(
                              clockInItem.clockOutDate +
                                " " +
                                clockInItem.clockOutTime
                            )
                          );
                        }}>Update Times</a
                      >
